...
def athread_write_kernel_launch_slave_source_code ( \
	...
	for i in range (len (list_reg_info)):
		...
		# malloc
		list_reg_code.append ("  AthreadHashNode* func" + str(i) + " = (AthreadHashNode*)\n" 
				+ "      libc_aligned_malloc(sizeof(AthreadHashNode));\n")
		# key
		list_reg_code.append ("  func" + str(i) + "->key = arr_char_to_arr_int (\n      \"" + register_info["key"] + "\");\n")
		# function pointer
		list_reg_code.append ("  func" + str(i) + "->fp = " + register_info["function"] + ";\n")
		# insert
		list_reg_code.append ("  kokkos_athread_hash_table_insert (func" + str(i) + ");\n")
