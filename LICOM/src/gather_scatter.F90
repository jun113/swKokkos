...
! !PUBLIC MEMBER FUNCTIONS:
   ...
   public :: gather_global,      &
             gather_global2,     & 
             scatter_global,     & 
             scatter_global_group_int,     & 
             scatter_global_group_dbl,     & 
             redistribute_blocks
...
!     allocate (msg_buffer(nx_block,ny_block))
     ...
     do n=1,nblocks_tot
       if (dst_dist%proc(n) > 0 .and. &
          ...
          !  write(*,*) "ori Send",mytid, n, dst_dist%proc(n)-1
     ...
     do n=1,nblocks_tot
       if (dst_dist%proc(n) == my_task+1) then
         ...
         this_block = get_block(n,n)
          ...
          !  write(*,*) "ori copy local",mytid,n, dst_dist%proc(n)
...
!-----------------------------------------------------------------------
     ...
     do n=1,nblocks_tot
       if (dst_dist%proc(n) == my_task+1) then
                  ! write(*,*) "ori recv",mytid,n, dst_dist%proc(n)
         nrecvs = nrecvs + 1
         dst_block = dst_dist%local_block(n)
...
!-----------------------------------------------------------------------
  ...
  subroutine scatter_global_group_int(ARRAY, ARRAY_G, num_task, dst_dist, &
                               field_loc, field_type)

! !DESCRIPTION:
!  This subroutine parallel scatters a global_sized array to a distributed array.

!  Note that this routine only is guaranteed to scatter correct values in the 
!  physical domain on each block.  Ghost cells are not filled correctly for tripole
!  boundary conditions due to the complexity of dealing with field
!  locations.  It is wise to call the boundary update routine after
!  each scatter call.
!
...
! !INPUT PARAMETERS:
   ...
   ! Number of tasks per group from which the array should be scattered
   integer (int_kind), intent(in) :: num_task
...
!-----------------------------------------------------------------------
   ...
   ! This task is the root task
   logical :: rootTask
   ! The root task of this task in the group
   integer (int_kind) :: myRootTask
...
!
!-----------------------------------------------------------------------
   if (mod(my_task, num_task) .eq. 0) then
      rootTask = .True.
   else
      rootTask = .False.
   endif
...
!     allocate (msg_buffer(nx_block,ny_block))
     ...
     do n=my_task + 2, my_task + num_task
       if (dst_dist%proc(n) > 0 .and. n <= nblocks_tot) then
      ! write (*,*), "opt send", my_task, n, dst_dist%proc(n)-1
        ...
        !  call MPI_ISEND(msg_buffer, nx_block*ny_block, &
        !                mpi_dbl, dst_dist%proc(n)-1, 3*mpitag_gs+n, &
        !                MPI_COMM_OCN, status, ierr)
         call MPI_SEND(msg_buffer, nx_block*ny_block, &
...
!     deallocate(msg_buffer)
     ...
     n = my_task + 1
    !  write(*,*) "opt copy local",mytid, n, dst_dist%proc(n)
...
!-----------------------------------------------------------------------
     ...
     allocate (rcv_request(num_task), &
               rcv_status(MPI_STATUS_SIZE, num_task))

     rcv_request = 0
     rcv_status  = 0

     nrecvs = 0
     myRootTask = my_task - mod(my_task, num_task) 

     n = my_task + 1
    ! write(*,*) "opt recv",mytid, n, dst_dist%proc(n)

     nrecvs = nrecvs + 1
     dst_block = dst_dist%local_block(n)
     call MPI_IRECV(ARRAY(1,1,dst_block), nx_block*ny_block, &
                   mpi_integer, myRootTask, 3*mpitag_gs+n, &
                   MPI_COMM_OCN, rcv_request(nrecvs), ierr)
...
!-----------------------------------------------------------------------
 ...
 end subroutine scatter_global_group_int

 subroutine scatter_global_group_dbl(ARRAY, ARRAY_G, num_task, dst_dist, &
                               field_loc, field_type)
...
!-----------------------------------------------------------------------
    ...
    ! write(*,*) "opt recv",mytid, n, dst_dist%proc(n)
     ...
     dst_block = dst_dist%local_block(n)
     call MPI_IRECV(ARRAY(1,1,dst_block), nx_block*ny_block, &
                   mpi_dbl, myRootTask, 3*mpitag_gs+n, &
                   MPI_COMM_OCN, rcv_request(nrecvs), ierr)
...
!-----------------------------------------------------------------------
 ...
 end subroutine scatter_global_group_dbl
