...
!-----------------------------------------------------------------------
   ...
   !    DEL4_DTN,DEL4_DTS,DEL4_DTE,DEL4_DTW,                          &
   !    DEL4_DUC,DUN,DUS,DUE,DUW,                      &
   !    DMC,DMN,DMS,DME,DMW,                      &
   !    DUM,                                      &
   ...
   real (r8), dimension (nx_block,ny_block,max_blocks_clinic) :: &
      DEL4_DTN,DEL4_DTS,DEL4_DTE,DEL4_DTW,                          &
      DEL4_DUC,DEL4_DUN,DEL4_DUS,DEL4_DUE,DEL4_DUW,                      &
      DEL4_DMC,DEL4_DMN,DEL4_DMS,DEL4_DME,DEL4_DMW,                      &
      DEL4_DUM,                                      &
      DEL4_AHF,              &! variable mixing factor for tracer   mixing
      DEL4_AMF,              &! variable mixing factor for momentum mixing
      DEL4_ratio_dxy         !changing mixing !YYQ add
...
!add 20200221
      do iblock = 1, nblocks_clinic
         DEL4_AMF(:,:,iblock) = (uarea(:,:,iblock)/   &
                          ((2.0D0*radius*PI/DBLE(IMT_GLOBAL))**2.0))**1.5
!add 20200221
         DEL4_ratio_dxy(:,:,iblock) = sqrt(uarea(:,:,iblock))/   &
...
!-----------------------------------------------------------------------
   ...
   ! wjl 20231123
   ! allocate(DEL4_DUC(nx_block,ny_block,nblocks_clinic), &
...
!-----------------------------------------------------------------------
      ...
      DEL4_DUN(:,:,iblock) = WORK1*UAREA_R(:,:,iblock)
      DEL4_DUS(:,:,iblock) = eoshift(WORK1,dim=2,shift=1)*UAREA_R(:,:,iblock)
      !*** DUN invalid now in northernmost ghost cell

      WORK1 = HUE(:,:,iblock)/HTS(:,:,iblock)

      DEL4_DUE(:,:,iblock) = WORK1*UAREA_R(:,:,iblock)
      DEL4_DUW(:,:,iblock) = eoshift(WORK1,dim=1,shift=-1)*UAREA_R(:,:,iblock)
...
!-----------------------------------------------------------------------
      ...
      DEL4_DUM(:,:,iblock) = -(DXKX + DYKY + c2*(KXU**2 + KYU**2))
      DEL4_DMC(:,:,iblock) = DXKY - DYKX
...
!-----------------------------------------------------------------------
      ...
      DEL4_DME(:,:,iblock) =  c2*KYU/(HTS(:,:,iblock) + &
                                 eoshift(HTS(:,:,iblock),dim=1,shift=-1))
      DEL4_DMS(:,:,iblock) = -c2*KXU/(HTW(:,:,iblock) + &
                                 eoshift(HTW(:,:,iblock),dim=2,shift=1))

   end do

   DEL4_DUC = -(DEL4_DUN + DEL4_DUS + DEL4_DUE + DEL4_DUW)               ! scalar laplacian
   DEL4_DMW = -DEL4_DME
   DEL4_DMN = -DEL4_DMS
...
!-----------------------------------------------------------------------
   ...
   ! wjl 20231123
   ! allocate(DEL4_DTN(nx_block,ny_block,nblocks_clinic),  &
   !          DEL4_DTS(nx_block,ny_block,nblocks_clinic),  &
   !          DEL4_DTE(nx_block,ny_block,nblocks_clinic),  &
   !          DEL4_DTW(nx_block,ny_block,nblocks_clinic))
   ...
   do iblock=1,nblocks_clinic
      ...
      DEL4_DTS(:,:,iblock) = WORK1*TAREA_R(:,:,iblock)
      DEL4_DTN(:,:,iblock) = eoshift(WORK1,dim=2,shift=-1)* &
                        TAREA_R(:,:,iblock)

      WORK1 = HTW(:,:,iblock)/HUN(:,:,iblock)

      DEL4_DTW(:,:,iblock) = WORK1*TAREA_R(:,:,iblock)
      DEL4_DTE(:,:,iblock) = eoshift(WORK1,dim=1,shift=1)* &
                        TAREA_R(:,:,iblock)
...
!-----------------------------------------------------------------------
      ...
      do i=this_block%ib-1,this_block%ie+1
          ...
          am_factor(i,j,bid) = sqrt(gradx1(i,j)**2+gradx2(i,j)**2+  & 
                          grady1(i,j)**2+grady2(i,j)**2)*dxdy/abs(am*DEL4_amf(i,j,bid))
      end do
      end do
   ...
   D2VK = c0
      ...
      do i=this_block%ib-1,this_block%ie+1
         ...
         D2UK(i,j)  =(CC (i,j    )*UMIXK(i  ,j  ) +        &
                      DEL4_DUN(i,j,bid)*UMIXK(i  ,j-1) +        &
                      DEL4_DUS(i,j,bid)*UMIXK(i  ,j+1) +        &
                      DEL4_DUE(i,j,bid)*UMIXK(i+1,j  ) +        &
                      DEL4_DUW(i,j,bid)*UMIXK(i-1,j  ))+        &
                     (DEL4_DMC(i,j,bid)*VMIXK(i  ,j  ) +        &
                      DEL4_DMN(i,j,bid)*VMIXK(i  ,j-1) +        &
                      DEL4_DMS(i,j,bid)*VMIXK(i  ,j+1) +        &
                      DEL4_DME(i,j,bid)*VMIXK(i+1,j  ) +        &
                      DEL4_DMW(i,j,bid)*VMIXK(i-1,j  ))
      end do
      end do
...
!
      ...
      do i=this_block%ib-1,this_block%ie+1
         D2VK(i,j)  =(CC (i,j    )*VMIXK(i  ,j  ) +        &
                      DEL4_DUN(i,j,bid)*VMIXK(i  ,j-1) +        &
                      DEL4_DUS(i,j,bid)*VMIXK(i  ,j+1) +        &
                      DEL4_DUE(i,j,bid)*VMIXK(i+1,j  ) +        &
                      DEL4_DUW(i,j,bid)*VMIXK(i-1,j  ))-        &
                     (DEL4_DMC(i,j,bid)*UMIXK(i  ,j  ) +        &
                      DEL4_DMN(i,j,bid)*UMIXK(i  ,j-1) +        &
                      DEL4_DMS(i,j,bid)*UMIXK(i  ,j+1) +        &
                      DEL4_DME(i,j,bid)*UMIXK(i+1,j  ) +        &
                      DEL4_DMW(i,j,bid)*UMIXK(i-1,j  ))
      end do
      end do
...
!20200221 YYQ
         !D2UK = AMF(:,:,bid)*D2UK
         !D2VK = AMF(:,:,bid)*D2VK
         D2UK = AM_FACTOR(:,:,bid)*DEL4_AMF(:,:,bid)*D2UK
         D2VK = AM_FACTOR(:,:,bid)*DEL4_AMF(:,:,bid)*D2VK
!20200221 YYQ
      elsewhere
         D2UK = c0
         D2VK = c0
   ...
   HDVK = c0
      ...
      do i=this_block%ib,this_block%ie
         HDUK(i,j)  = am*((CC (i,j    )*D2UK(i  ,j  ) +        &
                           DEL4_DUN(i,j,bid)*D2UK(i  ,j-1) +        &
                           DEL4_DUS(i,j,bid)*D2UK(i  ,j+1) +        &
                           DEL4_DUE(i,j,bid)*D2UK(i+1,j  ) +        &
                           DEL4_DUW(i,j,bid)*D2UK(i-1,j  ))+        &
                          (DEL4_DMC(i,j,bid)*D2VK(i  ,j  ) +        &
                           DEL4_DMN(i,j,bid)*D2VK(i  ,j-1) +        &
                           DEL4_DMS(i,j,bid)*D2VK(i  ,j+1) +        &
                           DEL4_DME(i,j,bid)*D2VK(i+1,j  ) +        &
                           DEL4_DMW(i,j,bid)*D2VK(i-1,j  )))
      end do
      end do
!  if (mytid ==716 ) then
...
!  end if 
      ...
      do i=this_block%ib,this_block%ie
         HDVK(i,j)  = am*((CC (i,j    )*D2VK(i  ,j  ) +        &
                           DEL4_DUN(i,j,bid)*D2VK(i  ,j-1) +        &
                           DEL4_DUS(i,j,bid)*D2VK(i  ,j+1) +        &
                           DEL4_DUE(i,j,bid)*D2VK(i+1,j  ) +        &
                           DEL4_DUW(i,j,bid)*D2VK(i-1,j  ))-        &
                          (DEL4_DMC(i,j,bid)*D2UK(i  ,j  ) +        &
                           DEL4_DMN(i,j,bid)*D2UK(i  ,j-1) +        &
                           DEL4_DMS(i,j,bid)*D2UK(i  ,j+1) +        &
                           DEL4_DME(i,j,bid)*D2UK(i+1,j  ) +        &
                           DEL4_DMW(i,j,bid)*D2UK(i-1,j  )))
      end do
      end do
...
!-----------------------------------------------------------------------
      ...
      CN = merge(DEL4_DTN(:,:,bid), c0, (k <= KMTN(:,:,bid)) .and.  &
                                   (k <= KMT (:,:,bid)))
      CS = merge(DEL4_DTS(:,:,bid), c0, (k <= KMTS(:,:,bid)) .and.  &
                                   (k <= KMT (:,:,bid)))
      CE = merge(DEL4_DTE(:,:,bid), c0, (k <= KMTE(:,:,bid)) .and.  &
                                   (k <= KMT (:,:,bid)))
      CW = merge(DEL4_DTW(:,:,bid), c0, (k <= KMTW(:,:,bid)) .and.  &
                                   (k <= KMT (:,:,bid)))
