...
! !PUBLIC MEMBER FUNCTIONS:
   ...
   public :: POP_BlocksCreate,        &
             ...
             POP_BlocksGetBlockInfo1, &
             POP_BlocksGetBlockInfo2, &
             POP_BlocksGetBlockInfo3
...
! !PUBLIC DATA MEMBERS:
   ...
   ! wjl 20240112
   ! used for C/C++ POP halo
   integer (i4), dimension(:), pointer, public :: POP_HaloMsgCreate_iGlobal
!EOP
!BOC
!-----------------------------------------------------------------------
!
...
!**********************************************************************
!BOP
subroutine get_allblocks_ij (inbr, jnbr, nbrID)
   integer (i4), intent(in) :: inbr, jnbr
! !OUTPUT PARAMETERS:
   integer (i4), intent(out) :: nbrID
   nbrID = allBlocksIJ(inbr, jnbr)
end subroutine get_allblocks_ij
...
!***********************************************************************
 subroutine POP_BlocksGetBlockInfo1(blockID, ib, ie, jb, je)
...
! !INPUT PARAMETERS:
   ...
   integer (i4), intent(in) :: blockID ! global block id for which parameters are requested

! !OUTPUT PARAMETERS:

   !parts of block data type to extract if requested
   integer (i4), intent(out) :: ib, ie, jb, je ! begin,end indices for physical domain
...
!----------------------------------------------------------------------
   ...
   ib = allBlocks(blockID)%ib
   ie = allBlocks(blockID)%ie
   jb = allBlocks(blockID)%jb
   je = allBlocks(blockID)%je

!----------------------------------------------------------------------
!EOC

 end subroutine POP_BlocksGetBlockInfo1

 subroutine POP_BlocksGetBlockInfo2(blockID, ib, ie, jb, je)
...
!----------------------------------------------------------------------
   ...
   ! if (allocated(POP_HaloMsgCreate_iGlobal)) then
   !    if (size(POP_HaloMsgCreate_iGlobal) /= size(allBlocks(blockID)%iGlobal)) then
   !       deallocate(POP_HaloMsgCreate_iGlobal)
   !    endif
   ! endif
   ! if (.not. allocated(POP_HaloMsgCreate_iGlobal)) then
   ! allocate(POP_HaloMsgCreate_iGlobal(size(allBlocks(blockID)%iGlobal)))
   ! endif
   ! do i = 1,size(allBlocks(blockID)%iGlobal)
   !    POP_HaloMsgCreate_iGlobal(i) = allBlocks(blockID)%iGlobal(i)
   ! enddo

   POP_HaloMsgCreate_iGlobal => allBlocks(blockID)%iGlobal

!----------------------------------------------------------------------
!EOC

 end subroutine POP_BlocksGetBlockInfo2

 subroutine POP_BlocksGetBlockInfo3(blockID, iBlock, jBlock)
...
! !OUTPUT PARAMETERS:
   ...
   integer (i4), intent(out) :: iBlock, jBlock ! cartesian i,j position for block
...
!----------------------------------------------------------------------
   if (blockID < 1 .or. blockID > POP_numBlocks) then
	 		! write(*, *), "POP_BlocksGetBlockInfo: invalid blockID"
         ! wjl 20240116
	 		write(*, *) "POP_BlocksGetBlockInfo: invalid blockID"
      return
   endif
...
!----------------------------------------------------------------------
   ...
   iBlock = allBlocks(blockID)%iBlock
   jBlock = allBlocks(blockID)%jBlock

!----------------------------------------------------------------------
!EOC

 end subroutine POP_BlocksGetBlockInfo3
!BOP
