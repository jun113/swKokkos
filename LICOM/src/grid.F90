...
!-----------------------------------------------------------------------
   ...
   !    wjl 20240225
   real (r8), dimension(:,:,:), allocatable, public :: &
      DXT, DYT,ht,hu              ! {x,y} spacing centered at T points
...
!-----------------------------------------------------------------------
   ...
   !    wjl 20240225
   if (.not. allocated(DXT)) then
      allocate(DXT(nx_block,ny_block,max_blocks_clinic), stat = ierr) 
   end if
   if (.not. allocated(DYT)) then
      allocate(DYT(nx_block,ny_block,max_blocks_clinic), stat = ierr) 
   end if
   if (.not. allocated(ht)) then
      allocate(ht(nx_block,ny_block,max_blocks_clinic), stat = ierr) 
   end if
   if (.not. allocated(hu)) then
      allocate(hu(nx_block,ny_block,max_blocks_clinic), stat = ierr) 
   end if
...
!EOC
   ...
   !    wjl 20240225
   if (allocated(DXT)) then
      deallocate(DXT) 
   end if
   if (allocated(DYT)) then
      deallocate(DYT) 
   end if
...
!EOC
 ...
 subroutine read_horiz_grid_group(horiz_grid_file, latlon_only, numTaskPerGroup)
...
!  !INPUT PARAMETERS:
   ...
   integer (i4), intent(in) :: numTaskPerGroup
...
!ZWP20131030
      ...
      call scatter_global_group_dbl(TLAT, TLAT_G, numTaskPerGroup, distrb_clinic, &
                          field_loc_center, field_type_scalar)
      call scatter_global_group_dbl(TLON, TLON_G, numTaskPerGroup, distrb_clinic, &
                          field_loc_center, field_type_scalar)
      ...
      call scatter_global_group_dbl(HUN, TLAT_G, numTaskPerGroup, distrb_clinic, &
                          field_loc_Sface, field_type_scalar)
      ...
      call scatter_global_group_dbl(DXT, TLON_G, numTaskPerGroup, distrb_clinic, &
                          field_loc_center, field_type_scalar)
...
!
      call scatter_global_group_dbl(DXU, TLON_G, numTaskPerGroup, distrb_clinic, &
                          field_loc_swcorner, field_type_scalar)
      ...
      call scatter_global_group_dbl(HUE, TLAT_G, numTaskPerGroup, distrb_clinic, &
                          field_loc_Wface, field_type_scalar)
...
!
      call scatter_global_group_dbl(DYU, TLON_G, numTaskPerGroup, distrb_clinic, &
                          field_loc_swcorner, field_type_scalar)
...
!-----------------------------------------------------------------------
      ...
      call scatter_global_group_dbl(DYT, TLON_G, numTaskPerGroup, distrb_clinic, &
                          field_loc_center, field_type_scalar)
      ...
      call scatter_global_group_dbl(HTS, TLAT_G, numTaskPerGroup, distrb_clinic, &
                          field_loc_Sface, field_type_scalar)
      call scatter_global_group_dbl(HTW, TLON_G, numTaskPerGroup, distrb_clinic, &
                          field_loc_Wface, field_type_scalar)
!
      if (rootTask) then
         read(25,rec=7,iostat=ioerr) TLAT_G
      ...
      call scatter_global_group_dbl(ANGLET, TLAT_G, numTaskPerGroup, distrb_clinic, &
                          field_loc_Center, field_type_angle)
      ...
      ! call scatter_global(ANGLET_debug, TLAT_G, master_task, distrb_clinic, &
      !                     field_loc_center, field_type_angle)
      ! if (mytid == 0) then
      ! write(*,* ) "==================="
      ! endif 
      ...
      ! do j=1,ny_block
      !    do i=1,nx_block
      !       if(ANGLET(i,j,1) .ne. ANGLET_debug(i,j,1)) then
      !          write (*,*), mytid, i, j, ANGLET(i,j,1), ANGLET_debug(i,j,1)
      !       endif
      !       ! if(hts(i,j,1) .ne. ANGLET_debug(i,j,1)) then
      !       !    write (*,*), mytid, i, j, hts(i,j,1), ANGLET_debug(i,j,1)
      !       ! endif
      !       ! if(TLAT(i,j,1) .ne. ANGLET_debug(i,j,1)) then
      !       !    write (*,*), mytid, i, j, TLAT(i,j,1), ANGLET_debug(i,j,1)
      !       ! endif
      !    end do
      ! end do
      ! call fortran_mpi_barrier
...
!EOC
 ...
 end subroutine read_horiz_grid_group
...
!        do j=1,jmt_global
!        do i=1,imt_global
!            if (kmt_g(i,j) > 0.5 )  kmt_g(i,j) = max(kmt_g(i,j),3)
!        end do
!        end do
!LPF20200321
!        do j=1672,1676
...
!-----------------------------------------------------------------------
      ...
      !                     field_loc_center, field_type_scalar)
      call scatter_global_group_int(KMT, KMT_G, numTaskPerGroup, distrb_clinic, &
                          field_loc_center, field_type_scalar)
   ! wjl 20240224
   !   do iblock = 1,  nblocks_clinic
...
!-----------------------------------------------------------------------
   ...
   !    wjl 20240225
      ...
      ARRAY_UGRID(i,j) = P25*ARRAY_TGRID(i  ,j  ) + &
                         P25*ARRAY_TGRID(i  ,j+1) + &
                         P25*ARRAY_TGRID(i-1,j  ) + &
                         P25*ARRAY_TGRID(i-1,j+1)
