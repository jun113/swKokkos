...
#ifndef KOKKOS_IMPL_ATHREAD_TASK_HPP
#define KOKKOS_IMPL_ATHREAD_TASK_HPP
...
class TaskQueueSpecialization<SimpleTaskScheduler<Kokkos::Athread, QueueType>> {
 public:
  ...
  using scheduler_type  = SimpleTaskScheduler<Kokkos::Athread, QueueType>;
  using member_type =
      TaskTeamMemberAdapter<HostThreadTeamMember<Kokkos::Athread>,
                            scheduler_type>;
  ...
  static void execute(scheduler_type const& scheduler) {
    ...
    while (0 < queue->m_ready_count) {
      ...
      if (end != task) {
        ...
        Kokkos::abort("TaskQueue<Athread>::execute ERROR: ready_count");
      }
    }
  }
...
extern template class TaskQueue<Kokkos::Athread,
                                typename Kokkos::Athread::memory_space>;
...
#endif /* #ifndef KOKKOS_IMPL_ATHREAD_TASK_HPP */
